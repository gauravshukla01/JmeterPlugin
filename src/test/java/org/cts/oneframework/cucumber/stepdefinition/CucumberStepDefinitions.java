...
public class CucumberStepDefinitions extends AbstractSteps {
	...
	public void book_a_bus_ticket() {
		getPageObjectManager().getHomePage().search().setSource(DefaultStepDefinition.currentIterationMap.get("from")).setDestination(DefaultStepDefinition.currentIterationMap.get("to")).selectOnwardDate(DateUtils.getCurrentDate("d-MMM yyyy")).selectReturnDate(DateUtils.getTomorrowDate("d-MMM yyyy")).searchBuses();
	}
